...
export default manejar(async (req, res, u) => {
  const { id } = req.query;
  if (req.method === "GET") {
    const r = await q("SELECT a.tipo, a.datos, p.usuario_id, p.id AS pid FROM archivos a JOIN proyectos p ON p.id = a.proyecto_id WHERE a.id = $1", [id]);
    const a = r.rows[0];
    if (!a) return res.status(404).end();
    if (!(await puedeVer(u, { id: a.pid, usuario_id: a.usuario_id }))) return res.status(403).end();
  ...
  if (req.method === "PUT") {
    ...
    const p = await q("SELECT id, usuario_id FROM proyectos WHERE id = $1", [pid]);
